...
function draw() {
  ...
  image(img[whichimage], 0, 0, width, height/2); // draw the new source
}
...
function randomize()
{
  var randthresh = 1; // 80% of pixels will be dead.
  img[whichimage].loadPixels(); // load pixels into memory
  img[1-whichimage].loadPixels(); // load pixels into memory
  for (var i = 0; i < img[whichimage].width; i++) {
    for (var j = 0; j < img[whichimage].height; j++) {
      var r = random(16)>randthresh; // true or false?
      var thecolor = color(r*245);
